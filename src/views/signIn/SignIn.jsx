...
function SignIn({history, setIsLogged}) {
    ...
    const sendLogin = async (values) => {
        try {
            let response = await fetch(process.env.REACT_APP_PROD_API_URL + "/user/login/check",
                {
                    method: "POST",
                    headers: {
            ...
            console.log(dataRequested, 'Data from Login')
            if(dataRequested.accessToken){
                await getUserData(dataRequested.accessToken)
                window.localStorage.setItem('accessToken', dataRequested.accessToken )
                setTimeout(() => {
                    history.push("/home")
                    setIsLogged(true)
                }, 2000)
    ...
    const getUserData = async (accessToken) =>{
        try {
            let response = await fetch(process.env.REACT_APP_PROD_API_URL + "/user/me",
                {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
            ...
            console.log(response, 'from getUserData')
            let dataRequested = await response.json()
            console.log(dataRequested, 'Data from Login')
            ...
            <div className="container" id="signUp">
                ...
                >
                    <Form className="container" onSubmit={formik.handleSubmit}>
                        ...
                        {showAlert && <Alert variant="success"> <Alert.Heading>Logged Successfully</Alert.Heading></Alert>}
                        <button
                            type="submit"
